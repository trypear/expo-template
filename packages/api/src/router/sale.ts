		...
		.mutation(async ({ ctx, input }) => {
			return ctx.db.transaction(async (trx) => {
				...
				const inventoryItems = await trx
					...
					.from(inventory)
					.where(
						or(...productIds.map(id => eqi(inventory.productId, id)))
					);
				...
				const saleItemsWithId = newSale ? saleItemsData.map(item => ({
					...item,
					saleId: newSale.id,
				})) : [];
	...
	getTopSellingProducts: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const topProducts = await ctx.db
				...
				.innerJoin(product, eqi(saleItem.productId, product.id))
				.where(
					or(...saleIds.map(id => eqi(saleItem.saleId, id)))
				)
